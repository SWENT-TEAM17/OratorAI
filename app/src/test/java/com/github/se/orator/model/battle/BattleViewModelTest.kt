...
class BattleViewModelTest {
    ...
    fun setUp() {
        ...
        doAnswer { invocation ->
            ...
            null
        }.whenever(mockUserProfileRepository).getUserProfile(any(), any(), any())
    ...
    fun `createBattleRequest should call storeBattleRequest`() = runTest {
        ...
        val expectedBattle = SpeechBattle(
    ...
    fun `listenForPendingBattles should update pendingBattles LiveData`() = runTest {
        ...
        whenever(mockBattleRepository.listenForPendingBattles(eq("testUser"), any())).thenAnswer { invocation ->
    ...
    fun `acceptBattle should update battle status to IN_PROGRESS`() = runTest {
        ...
        // Assert
        verify(mockBattleRepository).updateBattleStatus(eq(battleId), eq(BattleStatus.IN_PROGRESS), any())
    }
    ...
    fun `acceptBattle should not update battle status if getBattleById fails`() = runTest {
        ...
        verify(mockBattleRepository, never()).updateBattleStatus(eq(battleId), eq(BattleStatus.IN_PROGRESS), any())
    }
