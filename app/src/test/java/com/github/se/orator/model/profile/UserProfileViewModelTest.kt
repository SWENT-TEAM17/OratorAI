...
class UserProfileViewModelTest {
  ...
  fun `addNewestData should add new data to recentData and update user profile`() = runTest {
    // Arrange
    val newAnalysisData = AnalysisData(talkTimeSeconds = 100.0, talkTimePercentage = 50.0)
  ...
  fun `addNewestData should maintain a maximum size of 10 in recentData`() = runTest {
    ...
    for (i in 1..10) {
      val analysisData =
          AnalysisData(talkTimeSeconds = i.toDouble(), talkTimePercentage = i.toDouble())
    ...
    // Act
    val newAnalysisData = AnalysisData(talkTimeSeconds = 11.0, talkTimePercentage = 11.0)
    viewModel.addNewestData(newAnalysisData)
    testDispatcher.scheduler.advanceUntilIdle()
  ...
  fun `addNewestData should not add data when userProfile is null`() = runTest {
    ...
    // Act
    val newAnalysisData = AnalysisData(talkTimeSeconds = 50.0, talkTimePercentage = 50.0)
    viewModel.addNewestData(newAnalysisData)
    testDispatcher.scheduler.advanceUntilIdle()
  ...
  fun `updateMetricMean should calculate means and update user profile`() = runTest {
    ...
    val analysisDataList =
        listOf(
            AnalysisData(talkTimeSeconds = 10.0, talkTimePercentage = 20.0),
            AnalysisData(talkTimeSeconds = 20.0, talkTimePercentage = 30.0),
            AnalysisData(talkTimeSeconds = 30.0, talkTimePercentage = 40.0))
