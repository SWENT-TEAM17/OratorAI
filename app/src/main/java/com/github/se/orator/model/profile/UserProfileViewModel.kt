...
class UserProfileViewModel(internal val repository: UserProfileRepositoryFirestore) : ViewModel() {
    ...
    fun addFriend(friend: UserProfile) {
        ...
        if (currentUserProfile != null) {
            ...
            if (!currentUserProfile.friends.contains(friend.uid)) {
                val updatedFriendsList = currentUserProfile.friends.toMutableList().apply {
                    add(friend.uid)
                }
    ...
    fun uploadProfilePicture(uid: String, imageUri: Uri) {
        ...
        repository.uploadProfilePicture(
            ...
            onSuccess = { downloadUrl ->
                Log.d("UserProfileViewModel", "Profile picture uploaded successfully. Download URL: $downloadUrl")
    ...
    private fun updateUserProfilePicture(uid: String, downloadUrl: String) {
        Log.d("UserProfileViewModel", "Updating Firestore for user: $uid with profile picture URL: $downloadUrl")
        repository.updateUserProfilePicture(
            ...
            onFailure = { exception ->
                Log.e("UserProfileViewModel", "Failed to update profile picture URL in Firestore.", exception)
            }
        )
    }
