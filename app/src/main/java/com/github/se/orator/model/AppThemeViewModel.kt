...
class AppThemeViewModel(private val context: Context) {
    ...
    //private val preference: SharedPreferences = context.getSharedPreferences("isDark", Context.MODE_PRIVATE)
    private var deviceThemeIsDarkTheme = false

    /**
     * Load the theme from memory. If no value present, the theme of the device is used, and
     * saved as the current theme in memory.
     *
    ...
    /**
     * Switch the theme of the app, from dark to light or light to dark depending on the
     * current theme.
     */
    fun switchTheme(){
