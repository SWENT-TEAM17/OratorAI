...
class SymblApiClient(context: Context): VoiceAnalysisApi {
  ...
  private fun parseSentimentResponse(sentimentJson: JSONObject, onSuccess: (AnalysisData) -> Unit, onFailure: (Exception) -> Unit) {
    try {
      ...
      if (messagesArray.length() > 0) {
        ...
        onSuccess(AnalysisData(
          ...
          fillerWordsCount = -1,
            ...
            sentimentScore =  sentimentJson.getJSONArray("messages").getJSONObject(0).getJSONObject("sentiment").getJSONObject("polarity").getDouble("score")
        ))
  ...
  override fun getTranscription(
    ...
    val request = buildUrlPostRequest(
      url = "https://api.symbl.ai/v1/process/audio?enableAllInsights=true&insightTypes=filler_words",
      accessToken = accessToken,
      requestBody = requestBody
    )


    urlCallRequest( request = request,
      onSuccess = { response ->
  ...
  private fun fetchAnalysis(conversationId: String, onSuccess: (AnalysisData) -> Unit, onFailure: (Exception) -> Unit){
    urlCallRequest( request = buildUrlGetRequestWithHeader("https://api.symbl.ai/v1/conversations/$conversationId/messages?sentiment=true&enableAllInsights=true", accessToken),
      onSuccess = { response -> parseSentimentResponse(JSONObject(response), onSuccess, onFailure) },
   ...
   *
   * @return The status (String?) of the job : either "completed", "failed", "in_progress" or
   * null if the call to the api was unsuccessful.
   */
  private fun getJobStatus(jobId: String): String?{
    var status: String? = null

    urlCallRequestBlocking( request = buildUrlGetRequest("https://api.symbl.ai/v1/job/$jobId"),
      onSuccess = {response ->
        val jsonObject = JSONObject(response)
  ...
  private fun buildUrlGetRequest(url: String): Request {
    return Request.Builder().url(url).addHeader("Authorization", "Bearer $accessToken").get().build()
  }
   ...
   */
  private fun buildUrlPostRequest(url: String, accessToken: String?, requestBody: RequestBody): Request {
   ...
   */
  private fun urlCallRequest(request: Request, onSuccess: (String) -> Unit, onFailure: (Exception) -> Unit) {
    val client = OkHttpClient()
   ...
   * @param onFailure The function to be called on failure.
   */
  private fun urlCallRequestBlocking(request: Request, onSuccess: (String) -> Unit, onFailure: (Exception) -> Unit) {
    val client = OkHttpClient()
